...
>
> **最后更新**：2026-06-14（环境跑通 + 美股回测结果 ×3；GPU 镜像就绪[cu128/sm_120] + 死锁根治；RD-Agent
> 循环有 China-only/SDK bug，已用 `rdagent/custom/` 薄驱动绕过直接 qrun，见 §9.7/§9.8/§9.9/§10）

---

## 🧭 进 session 先读（跨会话交接）

> **现状一句话**：基础设施全就绪——环境、美股定制（连姊妹 `qlib` fork）、GPU 镜像（5090/cu128）、容器 NN
> 死锁已根治。已用 `rdagent/custom/us_qlib_backtest.py` 驱动出过 **3 份美股回测**（LightGBM×2 + RD-Agent
> 生成的 GRU），整条"LLM 生成→GPU 训练→美股回测"链路打通；**但都还不是稳健 alpha**（信号弱、run-to-run 噪声大）。
>
...
- **跑出新的本地运行经验/坑** → 记到 §9。
- **现状或下一步变了** → 更新顶部"🧭 进 session 先读"块 + §10 路线图状态表（跨会话交接靠这两处）。
- **里程碑/进度变化** → 同步更新记忆 `MEMORY.md` 索引那行 hook（每 session 自动加载，要能反映现状）。
- 每次改完，更新顶部"最后更新"日期。
